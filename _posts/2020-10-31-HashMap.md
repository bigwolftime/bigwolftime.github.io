...
![HashMap的类继承关系图(图源:知乎-美团技术团队)](https://img.zonheng.net/tech/map.jpg-original)
...
![JDK8 HashMap 的数据结构](https://img.zonheng.net/tech/aaaa.png-original)
...
![hash 的计算与索引的确定过程举例](https://img.zonheng.net/tech/hash.jpg-original)

#### 4. put()

JDK8 HashMap 的 put 流程可参考下图, 非常详细.

![HashMap put过程(图源:知乎-美团技术团队)](https://img.zonheng.net/tech/map_put.jpg-original)
...
![JDK7 rehash](https://img.zonheng.net/tech/reheah.jpg-original)

插入 (5, C) 键值对后, 此时节点数 size > threshold(扩容阈值), 会进入扩容过程. 上面图中剩余的 3 行就是扩容的过程.
**由此看出, 扩展之后元素要么在原来的位置, 要么在原位置上移动 2 次幂的位置**, 具体的解释如下图:

![](https://img.zonheng.net/tech/rehash_test_case.jpg-original)
...
![](https://img.zonheng.net/tech/rehash_loop_1.jpg-original)
...
![](https://img.zonheng.net/tech/rehash_loop_2.jpg-original)

5. `e.next = newTable[i]`. 导致 key(3).next 指向了 key(7), 由于此前 key(7) 已经指向了 key(3), 所以此处链表有了环. 如图

![环形链表](https://img.zonheng.net/tech/rehash_loop_3.jpg-original)
